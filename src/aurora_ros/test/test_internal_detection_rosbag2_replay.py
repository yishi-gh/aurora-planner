...
from generate_internal_detection_crossing import (  # noqa: E402
    PLANNING_REQUEST_ID,
    generate_internal_detection_crossing,
)


SCENARIO_DIR = Path(tempfile.mkdtemp(prefix="aurora_internal_detection_bag_", dir="/tmp"))
BAG_URI = SCENARIO_DIR / "internal_detection_crossing"
MANIFEST_PATH = generate_internal_detection_crossing(BAG_URI)
...
def generate_test_description():
    planner_node = launch_ros.actions.Node(
        ...
        name="aurora_planner_node_internal_detection_bag",
        output="screen",
        additional_env={
...
class TestInternalDetectionRosbag2Replay(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        rclpy.init()
    ...
    def setUp(self):
        self.node = rclpy.create_node("aurora_internal_detection_rosbag2_observer")
    ...
    def request_results(self, status):
        return [
            result
    ...
    def write_report(self, bag_start_ns, bag_end_ns, bag_message_count):
        report = {
            ...
            "internal_detection_replan_status_count": sum(
    ...
    def test_internal_detection_replays_through_tracker_and_risk_gate(self):
        ...
        try:
            self.wait_until(
                lambda: bool(self.request_results(PlanningResult.SUCCESS)),
                detail="initial successful internal-detection planning result",
            )
            self.assertTrue(
                any(
                    ...
                    for result in self.request_results(PlanningResult.SUCCESS)
                )
            )
            self.wait_until(
                lambda: bool(self.request_results(PlanningResult.VALIDATION_FAILED)),
                detail="internal detection dynamic collision rejection result",
            )
            collision_result = self.request_results(PlanningResult.VALIDATION_FAILED)[-1]
            ...
            self.wait_until(
                ...
                detail="internal detection active-replan status",
            )
            self.wait_until(
...
class TestInternalDetectionRosbag2ReplayShutdown(unittest.TestCase):
